...
def test_cookie_name_value():
    ...
    assert string == b'test1=alpha1; httponly'

def test_cookie_path():
    string = make_cookie('test2', 'alpha2', path='/path/to/location')

    assert string == b'test2=alpha2; Path=/path/to/location; httponly'

    string = make_cookie('test2', 'alpha2', path='/path/to/location',
            httponly=False)

    assert string == b'test2=alpha2; Path=/path/to/location'

def test_cookie_expire():
    string = make_cookie('test3', 'alpha3', expires=50)

    assert string == b'test3=alpha3; Max-Age=50; httponly'

def test_cookie_mac():
    string = make_cookie('test4', 'alpha4', mac_key='secret')

    secret_string = sha('%s%s' % ('alpha4', 'secret')).hexdigest().encode('UTF-8')

    assert string == b'test4=alpha4:' + secret_string + b'; httponly'

def test_cookie_domain():
  string = make_cookie('test5', 'alpha5', domain=".tiddlyspace.com")

  assert string == b'test5=alpha5; Domain=.tiddlyspace.com; httponly'
